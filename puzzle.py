...
#         self.size = size
#         self.actions = ['front', 'back', 'left', 'right', 'top', 'bottom'] #"ffront", "bback", "lleft", "rright", "ttop", "bbottom"]
#         if c:
#             self.d = c
...
#         genside = lambda c: [[c for _ in range(self.size)] for _ in range(self.size)]
#         self.__front__  = genside('W')
#         self.__back__   = genside('Y')
#         self.__top__    = genside('R')
#         self.__bottom__ = genside('O')
#         self.__left__   = genside('B')
#         self.__right__  = genside('G')
#         self.__sides__ = [self.front(), self.back(), self.left(), self.right(), self.top(), self.bottom()]
...
#         new_side = [[] for _ in range(self.size)]
...
class State:
    def __init__(self, size=3, c=None):
        self.size = size
        self.actions = ["F", "B", "L", "R", "U", "D", "F'", "B'", "L'", "R'", "U'", "D'"]
        self.cube = pc.Cube()
        self.solvedCube = pc.Cube()
    
    def copy(self):
        new_s = State()
        new_s.cube = self.cube.copy()
        return new_s
    ...
    def get_face(self, side):
        return [[str(x)[1].upper() for x in line] for line in self.cube.get_face(side)]

    def __str__(self):
        return "\nFRONT"  + str(self.get_face("F")) +\
               "\nBACK"   + str(self.get_face("B")) +\
               "\nLEFT"   + str(self.get_face("L")) +\
               "\nRIGHT"  + str(self.get_face("R")) +\
               "\nTOP"    + str(self.get_face("U")) +\
               "\nBOTTOM" + str(self.get_face("D"))
               
    def __hash__(self):
        return hash(self.__str__())
    
    def isGoalState(self):
        return self.cube == self.solvedCube
    
    def move(self, action):
        self.cube(action)
        
    def allSides(self):
        return [self.get_face(face) for face in ["F", "B", "L", "R", "U", "D"]]


def num_pieces_correct_side(state):
    correct = 0
    for side in state.allSides():
        # get middle piece
